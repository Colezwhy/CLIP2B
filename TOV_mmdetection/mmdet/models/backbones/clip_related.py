...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # the classic and simple bottleneck
...
class AttentionPool2d(nn.Module):
    def __init__(self, spacial_dim: int, embed_dim: int, num_heads: int, output_dim: int = None):
        super().__init__()
        # the spacial_dim is the parameter related to input resolution
        ...
        # original settings of a visual trasnformer

    def forward(self, x):
        B, C, H, W = x.shape
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]).permute(2, 0, 1)  # NCHW -> (HW)NC
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC  x.mean: 1NC

        cls_pos = self.positional_embedding[0:1, :]

        positional_dim, _, _ = x.shape
        ...
        positional_spa, ww = spatial_pos.shape

        transformation = nn.Upsample(size=positional_dim - 1, mode='nearest')
        # spatial_pos = spatial_pos.reshape(self.embed_dim, H*W).permute(1, 0)
        if positional_spa != positional_dim - 1:
            spatial_pos = torch.unsqueeze(spatial_pos, dim=0).permute(0, 2, 1)
            spatial_pos = transformation(spatial_pos)
            spatial_pos = spatial_pos.permute(0, 2, 1)
            spatial_pos = torch.squeeze(spatial_pos, dim=0)

        positional_embedding = torch.cat([cls_pos, spatial_pos], dim=0)

        # written in the format of cls token
        ...
        x = x.permute(1, 2, 0)  # B D HW+1
        global_feat = x[:, :, 0]  # the first one of -1 dim,being the class token.
        # when doing MHSA the output's corresponding position is not changed
        feature_map = x[:, :, 1:].reshape(B, -1, H, W)
        # take the rest part as the feat map
...
class CLIPResNetWithAttention(nn.Module):
    ...
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=None, att_level3=False,
                 baseline=False, frozen_weight=False, residual_num=4,  **kwargs):
        super().__init__()
        self.pretrained = pretrained
...
##########################added by Colez.
    def freeze_weights(self):
        # this func is used to freeze the pretrained backbones during training
        # the BottleNeck layers and AttentionPool2D layer will be frozen
        # by default, the first 3-stem of original Res50 will not be frozen.
        if self.frozen_weight:
            for i in range(1, self.residual_num + 1):
            ...
            print('No frozen weight in [CLIPRes_withatt] Module')
#############################
...
# default ResNet plus the defined AttentionPool layer


class LayerNorm(nn.LayerNorm):
    """Subclass torch's LayerNorm to handle fp16."""

    # the normalization method in transformer, which means the input of a layer is in a same distribution
    def forward(self, x: torch.Tensor):
        orig_type = x.dtype
...
class DropPath(nn.Module):
    ...
    # randomly deleting a branch of a deep residual Network, for better performance and non-overhead
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, drop_path=0.):
        super().__init__()
        # merging residual structure into the MHSA module
...
@BACKBONES.register_module()
class CLIPVisionTransformer(nn.Module):                                            #12
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=8, output_dim=512,
                 out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings = False, **kwargs):
        super().__init__()
        self.pretrained = pretrained
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        if isinstance(pretrained, str):
            ...
            if 'positional_embedding' in state_dict.keys():
                ...
                if self.positional_embedding.shape != state_dict['positional_embedding'].shape:
                    ...
                    # Interpolating the positional embedding from ImageNet default into spatial size.
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] 112
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width] 1
    ...
    # using conv layers to deform the channel depth.
...
#######################added by Colez.
    def freeze_weights(self):
        # this function is used to freeze the weights of pretrained backbone
        # here the positional embedding and basic Transformer's weights will be frozen
        ...
        if self.frozen_weight:
            ...
            print('No weight will be frozen in [CLIP_TextEncoder] Module')
##########################
...
#######################added by Colez.
    def freeze_weights(self):
        ...
        if self.frozen_weight:
            ...
            print('No weight will be frozen in [CLIP_TextContextEncoder] Module')

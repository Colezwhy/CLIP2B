...
ori_ann_needed = 1
# 根目录文件
root = '/home/junweizhou/Datasets/COCO/coco/train2017/'

gt_path = '/home/junweizhou/Point2BoxNet/P2BNet/TOV_mmdetection_cache/work_dir/coco/CLIP_coco/without_segloss5e-5_wdc5e-4_coco_1200_latest_pseudo_ann_CLIP.json'
ann_path = '/home/junweizhou/Datasets/COCO/coco/annotations/instances_train2017.json'
...
if ori_ann_needed == 1:
    print('Loading original coco gt boxes for visualization...')
    with open(ann_path, 'r') as file:
        ori_gt = json.load(file)
    print('ori gt boxes loaded successfully.')
...
gt_ann_bbox = defaultdict(list)
gt_ann_class = defaultdict(list)
...
# the original coco gt_bboxes in val2017 annotations
for x in ori_gt['annotations']:
    if x['image_id'] in img2id.values():
        gt_ann_bbox[x['image_id']].append(x['bbox'])
        gt_ann_class[x['image_id']].append(coco_id_name_map[x['category_id']])
...
for img_name in show_img[:100]:
    ...
    if gt_or_result == 1:
        ...
        bbox_ann = gt_ann_bbox[img_id]
        classes_ann = gt_ann_class[img_id]  ###
    else:
        bbox = id2bbox[img_id]
    ...
    if gt_or_result == 1:
        ...
        for i, bb in enumerate(bbox_ann):
            bb = [int(x) for x in bb]
            top_left, bottom_right = bb[:2], [bb[0] + bb[2], bb[1] + bb[3]]
            img = cv2.rectangle(img, tuple(top_left), tuple(bottom_right), color_ann,
                                1)  # blue(0,229,238) yellow(255,215,0) green(127,255,0) orange(255,140,0)

            label_text = classes_ann[i]
